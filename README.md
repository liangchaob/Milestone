...
```
- 约束：
  - 顶层必须是数组；每个里程碑必须包含 `title`、`desc` 和 `tasks`（数组）。
  - 每个任务至少包含 `text`；`desc` 与 `done` 可选。
  - 严禁额外字段、注释或除 JSON 以外的输出内容。
...
4) 里程碑建议 3-6 个；每个里程碑的任务建议 4-8 条；使用中文，内容具体可执行。
5) 不要包含 id、order 等字段；不要输出注释；确保 JSON 可解析。
